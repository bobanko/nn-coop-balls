...
//Global
let mutation_rate: number = 0.005;
let num_species: number = 25;
let max_acc_variation: number = 0.01;
let num_defenders: number = 6;
let num_lives: number = 3;
let graphics:boolean = false;
let stop:boolean = false;

let /*ArrayList<defender>*/ team = new Array< defender > ();
let /*ArrayList<enemy>*/ mafia = new Array < enemy > ();
let /*ArrayList<species>*/ speciesADN = new Array < species > ();
	...
	do {
		...
		if (lives == 0) //end of species
		{
			...
			if (species == num_species) //end of generation
			{
				...
				let /*Array<species>*/ ordered_speciesADN = []; /*new Array < Species > ()*/;
				for (let /*int*/ i = 0; i < num_species; i++) {
				...
				//new species
				let /*Array<species>*/ new_speciesADN = new Array < Species > ();
...
function/*void*/ update_defenders() {
	...
	for (let /*int*/ i = 0; i < team.size(); i++) {
		...
		let /*float*/ output /*[]*/ = speciesADN.get(species).calculateOutput(input);
